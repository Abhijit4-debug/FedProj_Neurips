...
def main_fedproj(args):
    ...
    for idx in range(args.num_users):
        ...
        clients.append(Client_FedProj(idx, copy.deepcopy(users_model[idx]), args.local_bs, args.local_ep,
                   args.lr, args.momentum, args.distill_T, args.device, train_dl_local, test_dl_local))
    ...
    for iteration in range(args.rounds):
        ...
        #     batch_public_ds = DatasetKDSplit(public_ds, batch_inds)
        # else:
        if args.memory_bs > 0:
        ...
        dropout_prob = 0.0  # Set dropout percentage (e.g., 0.2 means 20% dropout)
        ...
        #     # Compute loss trend over recent rounds
        #     if len(loss_locals) > 5:
        #         loss_trend = np.mean(loss_locals[-5:]) - np.mean(loss_locals[-10:-5])
        #     else:
        #         loss_trend = 0  # Not enough data yet

        #     # Adaptive projection decision
        #     if loss_trend < 0:  # Loss is decreasing, reduce projection probability
        #         is_proj = random.random() < 0.3  # Use projection 30% of the time
        #     elif loss_trend > 0:  # Loss is increasing, use projections more
        #         is_proj = True
        #     else:  # Stable loss, use original strategy
        #         is_proj = (iteration >= 10)
...
#         optimizer = torch.optim.SGD(net_glob.parameters(), lr=args.lr, momentum=args.momentum, weight_decay=0)
        ...
        for _ in range(args.distill_E):
            ...
            for batch_idx, d2 in enumerate(public_dl):
                ...
                kd_loss = kl_criterion(F.log_softmax(logits_student / T, dim=1), 
                                    F.softmax(teacher_logits / T, dim=1))
                
                reg = 0.0
                for param_index, param in enumerate(net_glob.parameters()):
                    reg += torch.norm((param - global_wavg[param_index].to(param.device)))**2  
            ...
            # print(f"Distill Epoch {_}:")
            # print(f"Logits Student Mean: {student_logits_all.mean().item()}, Std: {student_logits_all.std().item()}")
            # print(f"Teacher Logits Mean: {teacher_logits_all.mean().item()}, Std: {teacher_logits_all.std().item()}")
            # print(f"Avg KL Loss: {total_kd_loss / num_batches:.6f}")
            # print("-" * 50)
...
def run_fedproj(args, fname):
    ...
    for trial in range(args.ntrials):
        ...
        fedavg_ft_local, duration, f1, f2, f3, f4, f5, f6, ff1, ff2, ff3, ff4, ff5, ff6 = main_fedproj(args)
