...
def main_fedet(args):
    ...
    print('-'*40)
    print('FedET+ ::: FedET + Local FineTuning')
    sys.stdout.flush()
    ...
    print(f'-- FedET+ :: AVG Local Acc: {np.mean(local_acc):.2f}')
...
def run_FedET(args, fname):
    ...
    for trial in range(args.ntrials):
        ...
        FedET_ft_local, duration, hurt_cnt, forgetting_rounds, \
        f1, f2, f3, f4, f5, f6, ff1, ff2, ff3, ff4, ff5, ff6 = main_fedet(args)
        ...
        exp_FedET_ft_local.append(FedET_ft_local)
        ...
        print(f'-- FedET+ Fine Tuning Clients AVG Local Acc: {exp_FedET_ft_local[-1]:.2f}')
    ...
    template = '-- FedET+ Fine Tuning Clients AVG Local Acc: {:.2f} +- {:.2f}'
    print(template.format(np.mean(exp_FedET_ft_local), np.std(exp_FedET_ft_local)))
    ...
    template = ("-- FedET+: STD of Local Acc: {:3.2f} +- {:.2f}")
    print(template.format(np.mean(exp_ff1), np.std(exp_ff1)))

    template = ("-- FedET+: Top 10% Percentile of Local Acc: {:3.2f} +- {:.2f}")
    print(template.format(np.mean(exp_ff2), np.std(exp_ff2)))

    template = ("-- FedET+: Bottom 10% Percentile of Local Acc: {:3.2f} +- {:.2f}")
    print(template.format(np.mean(exp_ff3), np.std(exp_ff3)))

    template = ("-- FedET+: Avg Top 10% of Local Acc: {:3.2f} +- {:.2f}")
    print(template.format(np.mean(exp_ff4), np.std(exp_ff4)))

    template = ("-- FedET+: Avg Bottom 10% of Local Acc: {:3.2f} +- {:.2f}")
    print(template.format(np.mean(exp_ff5), np.std(exp_ff5)))

    template = ("-- FedET+: Difference Avg Top and Bottom 10% of Local Acc: {:3.2f} +- {:.2f}")
    ...
    with open(fname+'_results_summary.txt', 'a') as text_file:
        ...
        template = '-- FedET+ Fine Tuning Clients AVG Local Acc: {:.2f} +- {:.2f}'
        print(template.format(np.mean(exp_FedET_ft_local), np.std(exp_FedET_ft_local)), file=text_file)
