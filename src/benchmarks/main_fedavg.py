...
def main_fedavg(args):
    ...
    test_dl_global = get_dataset_global(args.dataset, args.datadir, batch_size=128,
                                        p_train=args.p_train, p_test=args.p_train, seed=args.seed)
    ...
    return (final_glob, avg_final_glob, best_glob, avg_final_local, avg_best_local, fedavg_ft_local, duration,
           forgetting_rounds, f1, f2, f3, f4, f5, f6, ff1, ff2, ff3, ff4, ff5, ff6)
...
def run_fedavg(args, fname):
    ...
    for trial in range(args.ntrials):
        ...
        fedavg_ft_local, duration, forgetting_rounds, \
        f1, f2, f3, f4, f5, f6, ff1, ff2, ff3, ff4, ff5, ff6 = main_fedavg(args)
    ...
    # template = "-- Avg Forgetting: {:.2f}"
    # print(template.format(np.mean(exp_forgetting_rounds)))
    ...
    with open(fname+'_results_summary.txt', 'a') as text_file:
        ...
        # template = "-- Avg Forgetting: {:.2f}"
        # print(template.format(np.mean(exp_forgetting_rounds)), file=text_file)
