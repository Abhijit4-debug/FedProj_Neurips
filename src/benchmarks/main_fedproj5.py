...
#         optimizer = torch.optim.SGD(net_glob.parameters(), lr=args.lr, momentum=args.momentum, weight_decay=0)
        ...
        #### Seting to local inference of global model
        public_dl = torch.utils.data.DataLoader(public_ds, batch_size=args.local_bs, shuffle=False, drop_last=False)
        ...
        for data, _,_,_ in public_dl:
            ...
            out, feat = net_glob(data)
            outs.append(out.detach().cpu())
            features.append(feat.detach().cpu())

        logits_inference = torch.cat(outs).numpy()
        features_inference = torch.cat(features).numpy()
        
        public_ds.set_logits(logits_inference)
        public_ds.set_features(features_inference)
...
def run_fedproj5(args, fname):
    ...
    for trial in range(args.ntrials):
        ...
        fedavg_ft_local, duration, f1, f2, f3, f4, f5, f6, ff1, ff2, ff3, ff4, ff5, ff6 = main_fedproj5(args)
