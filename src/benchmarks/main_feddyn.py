...
def main_feddyn(args):
    ...
    # --- Initialize global model and auxiliary variable h ---
    w_glob = {k: v.to(args.device).float() for k, v in copy.deepcopy(initial_state_dict).items()}
    h = {k: torch.zeros_like(v).to(args.device).float() for k, v in w_glob.items()}

    m = max(int(args.frac * num_users_FL), 1)  # number of clients per round
    ...
    for iteration in range(args.rounds):
        ...
        print('-- Average Train loss {:.3f}'.format(loss_avg))
        ...
        w_locals = []
        for idx in idxs_users:
            state_dict = copy.deepcopy(clients[idx].net.state_dict())
            state_dict = {k: v.to(args.device).float() for k, v in state_dict.items()}
            w_locals.append(state_dict)

        # --- Compute weighted average model (sum_theta) ---
        sum_theta = copy.deepcopy(w_locals[0])
        for k in sum_theta:
            sum_theta[k] = sum_theta[k] * fed_avg_freqs[0]

        for i in range(1, len(w_locals)):
            for k in sum_theta:
                sum_theta[k] += w_locals[i][k] * fed_avg_freqs[i]

        # --- Compute delta_theta = sum_theta - w_glob ---
        delta_theta = {k: sum_theta[k] - w_glob[k] for k in w_glob}

        # --- Update auxiliary variable h ---
        for k in h:
            h[k] -= 0.01 * (1.0 / num_users_FL) * delta_theta[k]

        # --- Compute global model update ---
        ww = copy.deepcopy(sum_theta)  # already weighted
        w_glob = {
            k: ww[k] - (1.0 / 0.01) * h[k]
            for k in ww
        }
...
def run_feddyn(args, fname):
    ...
    for trial in range(args.ntrials):
        ...
        feddyn_ft_local, duration, f1, f2, f3, f4, f5, f6, ff1, ff2, ff3, ff4, ff5, ff6 = main_feddyn(args)
