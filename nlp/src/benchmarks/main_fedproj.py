...
def main_fedproj(args):
    ...
    for cn in range(len(num_users)):
        ...
        for idx in range(num_users[cn]):
            ...
            clients_tmp.append(Client_FedProj(idx, users_model[cn][idx], args.local_bs, args.local_ep[cn], optim,
    ...
    for iteration in range(args.rounds):
        ...
        sys.stdout.flush()
        s_time = time.time()
        drop_out = 0.3  # 30% of the time, we drop projection

        for cn in range(len(idxs_users)):
            for idx in idxs_users[cn]:
                clients[cn][idx].set_state_dict(copy.deepcopy(w_glob[cn]))

                # 70% of the time, apply projection
                is_proj = random.random() > drop_out  # is_proj=True with 70% chance
                loss = clients[cn][idx].train(public_ds, is_proj=is_proj)
        ...
        for cn in range(len(num_users)):
            ...
            public_dl = torch.utils.data.DataLoader(public_ds, batch_size=128, shuffle=True, 
                                                    drop_last=False)
